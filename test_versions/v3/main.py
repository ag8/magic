...
class TangoAutoencoder(object):
    ...
    def model(self):
        ...
        KLD = -.5 * tf.reduce_sum(1. + enc_logsd - tf.pow(enc_mu, 2) - tf.exp(enc_logsd), axis=1)
        ...
        BCE = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=x_hat, labels=self.x), axis=1)
    ...
    def train(self, config):
        ...
        for ep in xrange(config.MAX_EPOCHS):
