...
class TangoEncoder(object):
    def __init__(self, sess):
        ...
        with tf.name_scope("cost"):
            ...
            # Now, the actual loss for training.
            # We want the distance between the latent representations of the lock and the key to be equal to their
            # overlap area--that is, we want to learn the latent space via a certain metricization.
            # Thus, our real loss is the (squared) difference between the distance between the two vectors,
            # and the actual overlap area.
            self.distance = tf.reduce_sum(tf.squared_difference(self.z, self.z_representation_of_key), 1)
        ...
        with tf.name_scope("Adam_optimizer_for_overlap"):
            # Use ADAM optimizer
            self.o_optimizer = \
                tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.training_loss)
    ...
    def partial_fit(self, X, overlap_areas):
        ...
        opt, o_opt, cost, training_loss, rec_loss, lat_loss, def_loss = self.sess.run((self.optimizer, self.o_optimizer, self.cost, self.training_loss, self.r_l, self.l_l, self.d_l),
                                  feed_dict={self.x: X,
