    ...
    def __init__(self, sess):
        ...
        with tf.name_scope("encoder_network"):
            ...
            for i in xrange(0, num_encoder_layers - 1):
                with tf.name_scope("encoder_layer_" + str(i) + "_weights"):
                    ...
                    variable_summaries(encoder_hi_weights)
            ...
            for i in xrange(1, num_encoder_layers):
                with tf.name_scope("encoder_layer_" + str(i) + "_biases"):
        ...
        # START DECODER WEIGHTS AND BIASES------------------------------------------------------------------------------
        ...
        # [BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_LAYERS].
        print("Wel"),
        self.reshaped_x = tf.reshape(self.x, [FLAGS.BATCH_SIZE, FLAGS.IMAGE_SIZE, FLAGS.IMAGE_SIZE, FLAGS.NUM_LAYERS])
        print("come!")
        ...
        lock_image = tf.zeros(shape=[24, FLAGS.IMAGE_SIZE, FLAGS.IMAGE_SIZE, 1])
        rotated_lock_image = tf.zeros(shape=[24, FLAGS.IMAGE_SIZE, FLAGS.IMAGE_SIZE, 1])
        key_image = tf.zeros(shape=[24, FLAGS.IMAGE_SIZE, FLAGS.IMAGE_SIZE, 1])
        print("Lock image shape: " + str(lock_image.get_shape()))
        print("come!")
        ...
        print("Wel"),
        print("LOCK IMAGE DO RESHAPE-A: " + str(lock_image.get_shape()))
        lock_image = tf.reshape(lock_image, [-1, FLAGS.IMAGE_SIZE * FLAGS.IMAGE_SIZE])
        print("LOCK IMAGE POSLE RESHAPE-A: " + str(lock_image.get_shape()))
