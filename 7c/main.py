...
def train(sess, batch_size=FLAGS.BATCH_SIZE, training_epochs=10, display_step=5):
    ...
    try:
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(total_batch):
                ...
                cost, rec_loss, lat_loss, def_loss = vae.partial_fit(batch_xs)

                if np.isnan(cost):
                    raise TrainingException("Got cost=nan")

                print(("Epoch: (" + str(epoch) + "/" + str(training_epochs) + ");").ljust(16) + (" i: (" + str(i) + "/" + str(
                    total_batch) + ").").ljust(16) + ("Current cost: " + str(cost) + "").ljust(30)),
                print(("[Reconstruction loss: " + str(rec_loss) + ";").ljust(32) + (" latent loss: " + str(
                    lat_loss) + ";").ljust(25) + ("deformation loss: " + str(def_loss) + ".").ljust(27) + "]")


                if i % 24 == 0:
                    send_cross_entropy_to_poor_mans_tensorboard(cost)
